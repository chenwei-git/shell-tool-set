...
then
  ...
  echo "main: input params error, please input two param, first is to_stop_app, second is to_start_app"
  exit 1
fi
...
if (( $? == 0 ))
then
  echo "main: start ${to_start_app} fail"
  exit 3
else
  echo "main: start ${to_start_app} success"
fi
